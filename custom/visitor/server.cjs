...
var allowedTopicPatterns = {
  ...
  education: /(?:教育|学历|学校|大学|学院|专业|毕业|学位|education|degree|university|college|academic\s+background)/iu,
  note: /(?:工作方式|工作风格|工程原则|工程判断|方法论|解决问题|协作方式|思考方式|原则|work\s+style|engineering\s+principles?|methodolog(?:y|ies)|problem[\s-]+solving|collaboration|approach)/iu,
  fit: /(?:岗位匹配|职位匹配|适合.{0,12}(?:岗位|职位)|胜任|为什么.{0,16}(?:录用|选择|岗位)|匹配度|fit\s+for|role\s+fit|job\s+fit|suitable\s+for|qualified\s+for|why\s+(?:hire|choose)|fit\s+the\s+(?:role|position))/iu
};
function refusedQuestion(code, message) {
  return { decision: "refuse", code, message, topics: [] };
}
function evaluatePublicQuestion(question, projection) {
  ...
  const topics = questionTopicOrder.filter(
    (topic) => allowedTopicPatterns[topic].test(normalized)
  );
  if (projection) {
    const normalizedLower = normalized.toLocaleLowerCase("en-US");
    for (const fact of projection.facts) {
      const fragments = fact.text.split(/[|｜。、:：;；·\s]+/gu).map((fragment) => fragment.trim()).filter(
        (fragment) => fragment.length >= 2 && fragment.length <= 80 && !/^\d+(?:[-—/]\d+)*$/u.test(fragment)
      );
      if (fragments.some(
        (fragment) => normalizedLower.includes(fragment.toLocaleLowerCase("en-US"))
      ) && !topics.includes(fact.topic)) {
        topics.push(fact.topic);
      }
    }
    topics.sort(
      (left, right) => questionTopicOrder.indexOf(left) - questionTopicOrder.indexOf(right)
    );
  }
  if (topics.length === 0) {
    return refusedQuestion(
...
var sensitivePublicTextPatterns = [
...
var publicHttpsUrl = external_exports.string().min(8).max(2048).refine((value) => {
  try {
    const url = new URL(value);
    return url.protocol === "https:" && !url.username && !url.password && !url.search && !url.hash && Boolean(url.hostname) && url.hostname.toLowerCase() !== "localhost" && !url.hostname.toLowerCase().endsWith(".local") && (0, import_node_net.isIP)(url.hostname.replace(/^\[|\]$/gu, "")) === 0 && !containsSensitivePublicText(value);
  } catch {
    return false;
  }
}, "must be a credential-free public HTTPS URL");
function publicText(maxLength) {
  return external_exports.string().trim().min(1).max(maxLength).refine(
    (value) => !containsSensitivePublicText(value),
    "must not contain contact or sensitive identity data"
  );
}
var shortText = publicText(200);
var summaryText = publicText(1200);
var localPublicImagePath = external_exports.string().trim().min(6).max(512).refine(
  (value) => /^\/[A-Za-z0-9._/-]+\.(?:avif|jpe?g|png|webp)$/i.test(value) && !value.startsWith("//") && !value.includes("..") && !value.includes("\\") && !value.includes("//"),
  "must be a same-origin PNG, JPEG, WebP, or AVIF path"
);
function requirePublicProvenance(shape) {
  return external_exports.strictObject({
    ...shape,
    sources: external_exports.array(publicHttpsUrl).max(8),
    ownerAttested: external_exports.literal(true).optional()
  }).superRefine((value, context) => {
    const provenance = value;
    if (provenance.sources.length === 0 && provenance.ownerAttested !== true) {
      context.addIssue({
        code: "custom",
...
var experienceSchema = requirePublicProvenance({
  role: shortText,
  organization: shortText,
  period: publicText(100),
  summary: summaryText
});
var projectSchema = requirePublicProvenance({
  name: shortText,
  summary: summaryText,
  evidence: external_exports.array(publicText(300)).min(1).max(12),
  coverImageUrl: localPublicImagePath.optional()
});
var educationSchema = requirePublicProvenance({
...
var noteSchema = requirePublicProvenance({
  title: shortText,
  summary: summaryText,
  evidence: external_exports.array(publicText(300)).min(1).max(8)
});
var publicKnowledgeTopic = external_exports.enum([
  "overview",
  "experience",
  "project",
  "skill",
...
var publicProfileSchema = external_exports.strictObject({
  version: external_exports.literal(1),
  displayName: publicText(80),
  headline: publicText(160),
  introduction: publicText(800),
  avatarUrl: localPublicImagePath.optional(),
  focusAreas: external_exports.array(publicText(80)).min(1).max(8),
  skillGroups: external_exports.array(
    external_exports.strictObject({
      label: publicText(80),
      items: external_exports.array(publicText(80)).min(1).max(24)
    })
  ).max(12).optional(),
  experience: external_exports.array(experienceSchema).max(16),
  projects: external_exports.array(projectSchema).max(16),
  education: external_exports.array(educationSchema).max(8).optional(),
  notes: external_exports.array(noteSchema).max(8).optional(),
  ...
  chat: external_exports.strictObject({
    welcomeTitle: publicText(160),
    welcomeBody: publicText(800),
    starterPrompts: external_exports.array(publicText(160)).min(1).max(8),
    enabledTopics: external_exports.array(publicKnowledgeTopic).min(1).max(6)
  }).optional()
});
...
var rejectedPiDecision = {
  decision: "refuse",
  code: "MODEL_OUTPUT_REJECTED",
  factIds: []
};
function parsePublicPiDecision(raw, projection) {
  ...
  return {
    ...
    factIds: projection.facts.filter((fact) => selected.has(fact.factId)).map((fact) => fact.factId)
  };
}
var outputSensitivePatterns = [
...
function guardPublicOutput(output) {
  if (!output.trim()) return PUBLIC_MODEL_OUTPUT_REFUSAL;
  const normalized = normalizePublicTextForSafety(output);
  return containsSensitivePublicText(normalized) || outputSensitivePatterns.some((pattern) => pattern.test(normalized)) ? PUBLIC_MODEL_OUTPUT_REFUSAL : output;
}
var sectionLabels = {
  overview: "\u4E2A\u4EBA\u4ECB\u7ECD",
  experience: "\u5DE5\u4F5C\u7ECF\u5386",
...
function renderPublicFacts(projection, factIds) {
  ...
  const sections = factTopicOrder.flatMap((topic) => {
    ...
    return facts.length > 0 ? [`${sectionLabels[topic]}\uFF1A
